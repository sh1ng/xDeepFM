...
"""
def scan(filename, feat_cnt, fieldnames):
    for row in csv.DictReader(open(filename), dialect='excel-tab', fieldnames=fieldnames):
...
def get_feat(featSet, filename, fieldNames):
    for row in csv.DictReader(open(filename), dialect='excel-tab',  fieldnames=fieldNames):
        for key, val in row.items():
            # print(key, val)
...
def convert_to_ffm(src_path, dst_path, fieldIndex, featIndex, fieldNames):
    out = open(dst_path, 'w')
    for row in csv.DictReader(open(src_path), dialect='excel-tab', fieldnames=fieldNames):
        feats = []
...
if __name__ == '__main__':
    input_path = "../../arboretum_benchmark/data/dac/train.txt"
    ...
    # 统计category feat的词频
    feat_cnt = defaultdict(lambda: 0)
    scan(train_file, feat_cnt, fields)
    scan(eval_file, feat_cnt, fields)
    scan(test_file, feat_cnt, fields)
    # 离散特征判断为长尾的阈值
    T = 4
    ...
    featSet = set()
    get_feat(featSet, train_file, fields)
    get_feat(featSet, test_file, fields)
    get_feat(featSet, eval_file, fields)
    ...
    convert_to_ffm(train_file, "../train.ffm", fieldIndex, featIndex, fields)
    convert_to_ffm(eval_file, "../eval.ffm", fieldIndex, featIndex, fields)
    convert_to_ffm(test_file, "../test.ffm", fieldIndex, featIndex, fields)
